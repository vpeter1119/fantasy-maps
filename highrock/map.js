...
var dragMarker = L.marker([-496,192],{draggable:true}).addTo(map);
// dragMarker is used to add new markers
dragMarker.on("dragend", (e) => {
	var coordinates = [e.target._latlng.lng,-e.target._latlng.lat];
	dragMarker.bindPopup(`<h4>Current coordinates: ${coordinates}<h4><p><form onSubmit="return false"><label for="name">Name:</label><br><input type="text" id="name" name="name"><br><label for="url">Url:</label><br><input type="text" id="url" name="url"><br><label for="desc">Description:</label><br><input type="text" id="desc" name="desc"><input hidden type="text" id="coords" value="${coordinates}"><br><br><button type="submit" onClick="submitGeoJsonData(); dragMarker.closePopup();">Save</button></form></p>`).openPopup();
})
...
{
	var popupData = `<h3><a href="${feature.properties.url}" target="_blank" rel="noopener noreferrer">${feature.properties.name}</a></h3>${feature.properties.desc}`;
